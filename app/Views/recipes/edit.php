...
<div class="container" style="max-width: 800px; padding-top: 3rem;">
    <h1>Edit Recipe</h1>
    <p style="color: var(--text-light); margin-bottom: 2rem;">Update your recipe details.</p>
        ...
        <div class="card-body">
            <form method="POST" action="<?php echo BASE_URL; ?>/recipes/edit">
                    ...
                    <div class="form-group">
                        ...
                        <select id="category_id" name="category_id" class="form-control" required>
                            ...
                            <?php foreach ($categories as $cat): ?>
                                <option value="<?php echo $cat['id']; ?>" <?php if ($recipe['category_id'] == $cat['id']) echo 'selected'; ?>><?php echo htmlspecialchars($cat['name']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="image_url">Image URL</label>
                        <input type="url" id="image_url" name="image_url" class="form-control" value="<?php echo htmlspecialchars($recipe['image_url'] ?? ''); ?>">
                    </div>
                </div>
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1rem;">
                    <div class="form-group">
                        <label for="preparation_time">Prep Time (min)</label>
                        <input type="number" id="preparation_time" name="preparation_time" class="form-control" value="<?php echo $recipe['preparation_time'] ?? 0; ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="cooking_time">Cook Time (min)</label>
                        <input type="number" id="cooking_time" name="cooking_time" class="form-control" value="<?php echo $recipe['cooking_time'] ?? 0; ?>" required>
                    </div>
                    <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <textarea id="ingredients" name="ingredients" class="form-control" rows="5" required><?php echo htmlspecialchars($recipe['ingredients'] ?? ''); ?></textarea>
                </div>

                <h3>Instructions</h3>
                <div class="form-group">
                    <label for="instructions">Instructions (one step per line)</label>
                    <textarea id="instructions" name="instructions" class="form-control" rows="7" required><?php echo htmlspecialchars($recipe['instructions'] ?? ''); ?></textarea>
                </div>
